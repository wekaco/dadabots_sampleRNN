...
# Handling WHICH_SET
from datasets.dataset_conditioning import music_train_feed_epoch as train_feeder
from datasets.dataset_conditioning import music_valid_feed_epoch as valid_feeder
from datasets.dataset_conditioning import music_test_feed_epoch  as test_feeder
...
TRAINING_FEATURES = []
# This is the feature matrix which is used during generation 
# It has shape (num_feature_frames, num_features)
GENERATE_FEATURES = []

def prepare_features(training_npy, generate_npy):
    global N_FEATURES, TRAINING_FEATURES, GENERATE_FEATURES
    [features_frame_rate_training, _training_features] = load_feature_npy(training_npy)
    [features_frame_rate_generate, _generate_features] = load_feature_npy(generate_npy)
    # frame rates don't have to agree
    # but the number of features must agree
    assert(_training_features.shape[1] == _generate_features.shape[1])
    # Okay we know the # of features
    N_FEATURES = _training_features.shape[1]
    # the length of the training_features training set size

# Load in the local conditioning features from the numpy file 
def load_feature_npy(npy):
    # That we want to use in generating
    # the numpy file should have in [0] the feature_frame rate (hop size) in seconds 
    # in [1] the actual feature data
    # features.shape should be (num_feature_frames, num_features)
    feature_data = numpy.load(training_npy)
    feature_frame_rate = feature_data[0] # in seconds
    features = feature_data[1].astype('float32') 
    print "features.shape", features.shape # (frames, features )
    return [feature_frame_rate, features]
"""
...
def frame_level_rnn(input_sequences, h0, reset, features):
    """
...
if args.debug:
    ...
    features.tag.test_value = numpy.zeros((BATCH_SIZE, SEQ_LEN, N_FEATURES), dtype='float32')
...
frame_level_outputs, new_h0 =\
    frame_level_rnn(input_sequences, h0, reset, features)
...
train_fn = theano.function(
    [sequences, h0, reset, mask, features],
    [cost, new_h0],
    updates=updates,
...
frame_level_generate_fn = theano.function(
    [sequences, h0, reset, features],
    frame_level_rnn(sequences, h0, reset, features),
    on_unused_input='warn'
)
...
def generate_and_save_samples(tag):
    ...
    for i in xrange(N_SEQS):
        samp = samples[i]
        if Q_TYPE == 'mu-law':
            from datasets.dataset_conditioning import mu2linear
            samp = mu2linear(samp)
        elif Q_TYPE == 'a-law':
...
def monitor(data_feeder):
    ...
    for _seqs, _reset, _mask, _feats in _data_feeder:
        _cost, _h0 = test_fn(_seqs, _h0, _reset, _mask, _feats)
        _costs.append(_cost)
